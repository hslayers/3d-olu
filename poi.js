...
export default {
    getPois: function (map, utils, rect) {
        ...
        var extents = `POLYGON ((${prepareCords(rect[0])}, ${prepareCords(rect[1])}, ${prepareCords(rect[2])}, ${prepareCords(rect[3])}, ${prepareCords(rect[0])}))`;
        //console.log(extents);
        var q = 'https://www.foodie-cloud.org/sparql?default-graph-uri=&query=' + encodeURIComponent(`PREFIX geo: <http://www.opengis.net/ont/geosparql#> 
                ...
                WHERE {?poi geo:asWKT ?wkt . 
                    FILTER(bif:st_may_intersect(bif:st_geomfromtext("${extents}"), ?wkt)).
